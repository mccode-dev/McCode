\subsection*{The single crystal component}
...
%$\sigma_\mathrm{inc}$. The absorption cross-section is supplied by the user at
%2200~m/s, so the actual cross-section for a neutron of velocity $v$ is
%$\sigma_\mathrm{abs} = \sigma_{2200} \frac\mathrm{2200~m/s}{v}$.
%
...
%\left(\frac{d\sigma}{d\Omega}\right)_\mathrm{coh.el.} =
...
%individual unit cell, and $\boldsymbol{\kappa} (= \boldsymbol{k}_i - \boldsymbol{k}_f)$
%is the scattering vector. $\delta(\boldsymbol{x})$ is a 3-dimensional delta
%function in reciprocal space,
%so for given incoming wave vector $\boldsymbol{k}_i$ and lattice vector
%${\boldsymbol{\tau}}$, only a single final wave vector $\boldsymbol{k}_f$ is allowed.
...
%\begin{figure}[t]
%  \begin{center}
%    \psfrag{ki}[r][r]{$\boldsymbol{k}_\mathrm{i}$}
%    \psfrag{kf}[l][l]{$\boldsymbol{k}_\mathrm{f}$}
...
%  e^{-\boldsymbol{x}^\mathrm{T} D \boldsymbol{x}}
%\end{equation}
...
%$U^{-1} = U^\mathrm{T}$. The translation between global and local
%coordinates is
%$$ \boldsymbol{x} = U(\boldsymbol{y} + \boldsymbol{\tau}) \qquad
%   \boldsymbol{y} = U^\mathrm{T} \boldsymbol{x} - \boldsymbol{\tau} $$
%
%The expression for the 3-dimensional Gaussian in global coordinates is
%\begin{equation}
%  G(\boldsymbol{y}) =
%  \frac{1}{(\sqrt{2\pi})^3}\frac{1}{\sigma_1\sigma_2\sigma_3}
%  e^{-(U(\boldsymbol{y}+\boldsymbol{\tau}))^\mathrm{T} D (U(\boldsymbol{y}+\boldsymbol{\tau}))}
%\end{equation}
...
%  compute the total coherent cross-section $\sigma_\mathrm{coh}$ (see
%  below), the absorption cross-section $\sigma_\mathrm{abs} = \sigma_\mathrm{
%  2200} \frac{\mathrm{2200~m/s}}{v}$, and the total cross-section
%  $\sigma_\mathrm{tot} = \sigma_\mathrm{coh}+\sigma_\mathrm{inc}+\sigma_\mathrm{abs}$.
%\item\label{enum:crystal-3} The transmission probability is
%  $\exp(- \frac{\sigma_\mathrm{tot}}{V_0}\ell)$ where $\ell$ is the length of
...
%
%The final wave vector $\boldsymbol{k}_\mathrm{f}$ must lie on the
...
%$-\boldsymbol{k}_\mathrm{i}$ and the reciprocal lattice point
%$\boldsymbol{\tau}$. Since the radius of the Ewald sphere is $k_\mathrm{
%  i}$, this point is
%$$ \boldsymbol{o}=(k_\mathrm{i}/\rho - 1)\boldsymbol{\rho} - \boldsymbol{\tau} $$
%where $\boldsymbol{\rho} = \boldsymbol{k}_\mathrm{i} - \boldsymbol{\tau}$.
%\begin{figure}[t]
%  \begin{center}
%    \psfrag{ki}[r][r]{$\boldsymbol{k}_\mathrm{i}$}
%    \psfrag{kf}[l][l]{$\boldsymbol{k}_\mathrm{f}$}
...
%$\boldsymbol{k}_\mathrm{f}(\boldsymbol{t}) = \boldsymbol{k}_\mathrm{i} +
%\boldsymbol{P}(\boldsymbol{t})$. The value of the 3-dimensional Gaussian
%for this $\boldsymbol{k}_\mathrm{f}$ is
%\begin{equation}
%  \label{eq:crystal-gauss-t-1}
%  G(\boldsymbol{x}(\boldsymbol{t})) =
%  \frac{1}{(\sqrt{2\pi})^3}\frac{1}{\sigma_1\sigma_2\sigma_3}
%  e^{-\boldsymbol{x}(\boldsymbol{t})^\mathrm{T} D \boldsymbol{x}(\boldsymbol{t})}
%\end{equation}
%where $\boldsymbol{x}(\boldsymbol{t}) = \boldsymbol{\tau} -
%(\boldsymbol{k}_\mathrm{i} - \boldsymbol{k}_\mathrm{f}(\boldsymbol{t}))$ is
...
%  e^{-(\boldsymbol{t}-\boldsymbol{t}_0)^\mathrm{T} M
%    (\boldsymbol{t}-\boldsymbol{t}_0)}
%\end{equation}
%where $M = B^\mathrm{T} D B$ is a $2 \times 2$ symmetric and positive
%definite matrix, $\boldsymbol{t}_0 = -M^{-1}B^\mathrm{T} D \boldsymbol{o}$
%is a 2-vector, and $\alpha = -\boldsymbol{t}_0^\mathrm{T} M
%\boldsymbol{t}_0 + \boldsymbol{o}^\mathrm{T} D \boldsymbol{o}$ is a real
...
%defined by $M$.
%
%To choose $\boldsymbol{k}_\mathrm{f}$ we sample $\boldsymbol{t}$ from the
...
%L^\mathrm{T} = \frac{1}{2}M^{-1}$ and is possible since $M$ is symmetric
...
%    &=& \frac{1}{2\pi}
%      e^{-\frac{1}{2}\boldsymbol{g}^\mathrm{T}\boldsymbol{g}} d\boldsymbol{g} \\
%    &=& \frac{1}{2\pi}\frac{1}{\det L}
%      e^{-\frac{1}{2}(L^{-1}(\boldsymbol{t}-\boldsymbol{t}_0))^\mathrm{T}
%          (L^{-1}(\boldsymbol{t}-\boldsymbol{t}_0))} d\boldsymbol{t} \\
%    &=& \frac{1}{2\pi}\frac{1}{\det L}
%      e^{-(\boldsymbol{t}-\boldsymbol{t}_0)^\mathrm{T}
...
%$\boldsymbol{k}'_\mathrm{f} = \boldsymbol{k}_\mathrm{i} +
%\boldsymbol{P}(\boldsymbol{t})$ and
%$\boldsymbol{k}_\mathrm{f} = (k_\mathrm{i}/k'_f)\boldsymbol{k}'_\mathrm{f}$ to
%normalize the length of $\boldsymbol{k}_\mathrm{f}$ to correct for the
...
%$$
%\sigma_\mathrm{coh} = \int_\mathrm{Ewald}
%\left(\frac{d\sigma}{d\Omega}\right)_\mathrm{coh.el.} d\Omega
%$$
...
%  \sigma_{\mathrm{coh},\boldsymbol{\tau}} &=& \int N\frac{(2\pi)^3}{V_0}
%        G(\boldsymbol{\tau} - \boldsymbol{\kappa})
...
%         |F_{\boldsymbol{\tau}}|^2
%         \int e^{-(\boldsymbol{t}-\boldsymbol{t}_0)^\mathrm{T} M
%         (\boldsymbol{t}-\boldsymbol{t}_0)}
%         d\boldsymbol{t} \\
%  &=& \det(L) \frac{1}{\boldsymbol{k}_i^2} N\frac{(2\pi)^{3/2}}{V_0}
%         \frac{e^{-\alpha}}{\sigma_1\sigma_2\sigma_3}
%         |F_{\boldsymbol{\tau}}|^2
%         \int e^{-\frac{1}{2}\boldsymbol{g}^\mathrm{T}\boldsymbol{g}}
%         d\boldsymbol{g} \\
...
%         \frac{e^{-\alpha}}{\sigma_1\sigma_2\sigma_3}
%         |F_{\boldsymbol{\tau}}|^2 \\
%  \sigma_\mathrm{coh} &=& \sum_{\boldsymbol{\tau}} \sigma_{\mathrm{coh},\boldsymbol{\tau}}
%\end{eqnarray}
...
%
%If the user has choosen a fixed transmission probability $f(\mathrm{
%  transmit}) = p_\mathrm{transmit}$, the neutron weight must be adjusted by
%$$ \pi(\mathrm{transmit}) = \frac{P(\mathrm{transmit})}{f(\mathrm{transmit})}
%$$
%where $P(\mathrm{transmit}) = \exp(-\frac{\sigma_\mathrm{tot}}{V_0}\ell)$ is
%the physical transmission probability. Likewise, for non-transmission
%the adjustment is
%$$ \pi(\mathrm{no~transmission}) = \frac{1-P(\mathrm{transmit})}{1-f(\mathrm{transmit})}.
%$$
%
%Absorption is never explicitly simulated, so the Monte Carlo probability
%of coherent or incoherent scattering is
%$f(\mathrm{coh})+f(\mathrm{inc}) = 1$.
%The physical probability of coherent or incoherent scattering is
%$$ P(\mathrm{coh})+P(\mathrm{inc}) = \frac{\sigma_\mathrm{coh} + \sigma_\mathrm{
%    inc}}{\sigma_\mathrm{tot}}, $$
%so again a weight adjustment $\pi(\mathrm{coh}|\mathrm{inc}) = \Pi(\mathrm{
%    coh}|\mathrm{inc})/f(\mathrm{coh}|\mathrm{inc})$ is needed.
%
%When choosing the reciprocal lattice vector $\boldsymbol{\tau}$ to
%scatter from, the relative probability for $\boldsymbol{\tau}$ is
%$r_{\boldsymbol{\tau}} = \sigma_{\mathrm{
...
%$$
%whereas the physical probability is $P(\boldsymbol{\tau}) = \sigma_{\mathrm{
%    coh},\boldsymbol{\tau}}/\sigma_\mathrm{coh}$. A weight adjustment is
%thus needed of
%$$
%\pi(\boldsymbol{\tau}) =
% \frac{P(\boldsymbol{\tau})}{f(\boldsymbol{\tau})} =
% \frac{\sigma_{\mathrm{coh},\boldsymbol{\tau}}
%  \sum_{\boldsymbol{\tau}} r_{\boldsymbol{\tau}}}
% {\sigma_\mathrm{coh} \; r_{\boldsymbol{\tau}}}.$$
%
