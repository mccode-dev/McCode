\subsection*{A continuous source
with a Maxwellian spectrum}
\label{source-maxwell}
\index{Sources!Continuous source
with a Maxwellian spectrum}

%\component{Source\_Maxwell\_3}{System}{ $h$, $w$, $d_\textrm{foc}$, $xw$, $yh$, $\lambda_\textrm{low}$, $\lambda_\textrm{high}$, $I_1$, $T_1$}{$I_2$, $T_2$, $I_3$, $T_3$}{Validated. t=0}
%\mcdoccomp{sources/Source_Maxwell_3.parms}
...
calculated according to Eq. (\ref{e:flux-p0}), with $\Psi(\lambda)$ replaced
...
%$\lambda_2$ is thus
%\begin{equation}
%I_\textrm{tot} = \Omega A \int_{\lambda_1}^{\lambda_2} I_1 M(\lambda,T_1) d\lambda.
%\end{equation}
%In a Monte Carlo integration, the observed intensity becomes
%\begin{equation}
%I_\textrm{MC} \approx N_\textrm{MC} \int p(\lambda) \pi_1(\lambda) d\lambda ,
%\end{equation}
%where $N_\textrm{MC}$ is the number of Monte Carlo steps.
%We here choose the wavelength from a uniform distribution between the two
%limits, giving $p(\lambda)=1/(\lambda_2-\lambda_1)$.
%To fulfill $I_\textrm{tot} = I_\textrm{MC}$ we need to have
%\begin{equation}
%\pi_1(\lambda) = \Omega A (\lambda_2-\lambda_1) I_1 M(\lambda,T_1) / N_\textrm{MC} .
%\end{equation}
%
...
The component \textbf{Source\_gen} (see section \ref{source-gen})
works on the same principle, but provides more options concerning
wavelength/energy range specifications, shape, etc.

Some example transmission data files usable alongside sources like this one
are given in Table~\ref{t:filter-transmission-data}.
