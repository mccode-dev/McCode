...
A process does, however, use its rotation relative to ABSOLUTE to rotate
its local frame. This can be used to create a small offset between two
\texttt{Single\_crystal\_process} instances to describe a simple version
of twinning. For that reason, always set the process rotation relative to
ABSOLUTE.
...
\begin{table}[htb!]
  \begin{center}
    {\let\my=\\
    \begin{tabular}{|>{\raggedright\arraybackslash}p{0.28\textwidth}|>{\raggedright\arraybackslash}p{0.56\textwidth}|}
      \hline
      ...
      \texttt{target\_index}/\newline\texttt{target\_x,y,z}, \texttt{focus\_aw}/\texttt{focus\_ah}, \texttt{focus\_xw}/\texttt{focus\_xh}, \texttt{focus\_r} & Standard \MCS focusing parameters, used by any process assigned to this volume's material that supports focusing. \\
      \hline
    \end{tabular}
...
available for use with the OpenACC build (see the User Manual's chapter
on running \MCS, section on GPU acceleration); the regular
...
\item \textbf{GPU.} The default \texttt{Union\_master} is CPU-only
  ...
  (see the User Manual's chapter on running \MCS, section on GPU
  acceleration).
\end{itemize}
...
examples for how to browse these. A number of
