...
Cylindrical monitor on the x-z plane outputting a series of energy-planes in a subset of reciprocal space, spanned by scattering vectors qa(x,z) and qb(x,z).

The radius and yheight parameters are not used for propagation, but only to define the outgoing divergence limit considered when estimating k\_f
...
Example: Sqq\_w\_monitor(filename="output",radius=1, yheight=0.05, Emin=0,Emax=5,nE=11,nqa=100,nqb=100,qamin=1,qamax=10,qbmin=1qbmax=10, vix="vix", viy="viy", viz="viz") AT (0,0,0) RELATIVE sample
...
\subsection*{Links}
\begin{itemize}
  \item Component source code found in file \texttt{Sqq\_w\_monitor.comp}.
\end{itemize}
\IfFileExists{monitors/Sqq_w_monitor_static.tex}{\input{monitors/Sqq_w_monitor_static.tex}}{}