\section{The \texttt{Union\_conditional\_standard} McStas Component}
...
\midrule
\endhead
target\_loggers & string & Comma seperated list of loggers this conditional should affect & "NULL" \\
master\_tagging & 1 & Apply this conditional to the tagging system in next master & 0 \\
tagging\_extend\_index & 1 & Not currently used & -1 \\
time\_min & s & Min time, if not set ignored & 0 \\
time\_max & s & Max time, if not set ignored & 0 \\
E\_min & meV & Max energy, if not set ignored & 0 \\
E\_max & meV & Min energy, if not set ignored & 0 \\
total\_order\_min & 1 & Min scattering order, if not set ignored & 0 \\
