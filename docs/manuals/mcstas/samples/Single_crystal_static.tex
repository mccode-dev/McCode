...
\textit{mosaic}, \emph{or} anisotropically as \textit{mosaic\_a}
(rotation around lattice vector $\vec a$), \textit{mosaic\_b} (rotation around lattice vector $\vec
b$), and \textit{mosaic\_c} (rotation around lattice vector $\vec c$); in all
...
cross-section may be given as \textit{sigma\_abs} and
\textit{sigma\_inc} (in barns), with default of zero; and
