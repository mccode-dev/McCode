...
&\times&
  \sum_{\tau,q,p} \frac{(\boldsymbol{\kappa} \cdot \textbf{e}_{q,p})^2}
                       {\omega_{q,p}}
...
$\sum_{p} (\boldsymbol{\kappa} \cdot \textbf{e}_{q,p})^2 = \kappa^2$.
...
The $\boldsymbol{\kappa}$ sum can now be removed by
...
\frac{d^2\sigma'}{d\Omega dE_\textrm{f}}
  ...
  \delta(\omega\pm\omega_{q}) \delta(\boldsymbol{\kappa}\pm\textbf{q})
   d^3\textbf{q} \nonumber \\
