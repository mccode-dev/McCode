\section{The \texttt{TOFSANSdet} McStas Component}
...
\begin{itemize}
  \item \textbf{Author:} Henrich Frielinghaus, FZJuelich
...
centol &  & tolerance of center determination (if center larger than centol*calculated\_center then set back to theory) & 0.1 \\
inttol &  & tolerance of intensity            (if primary beam intensity smaller than inttol*max\_intensity then discard data) & 0.0001 \\
qcal &  & calibration of intensity (cps) to width of q-bin (non\_zero = yes, zero = no) & 1 \\
...
\subsection*{Links}
\begin{itemize}
  \item Component source code found in file \texttt{TOFSANSdet.comp}.
\end{itemize}
\IfFileExists{contrib/TOFSANSdet_static.tex}{\input{contrib/TOFSANSdet_static.tex}}{}