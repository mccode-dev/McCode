\section{The \texttt{ViewModISIS} McStas Component}
ISIS Moderators
...
dist = 1.7, focus\_xw = 0.094, focus\_yh = 0.094, modPosition=0,

\begin{verbatim}
xw=0.12,yh=0.12)
\end{verbatim}

MERLIN simulation. modPosition=0 so initial surface is at (near to) the moderator face. In this example, focus\_xw and focus\_yh are chosen to be identical to the shutter opening dimension. dist = 1.7 is the 'real' distance to the shutter front face =\textgreater{} This is TimeOffset value (=170 [cm]) from TS1\_S04\_Merlin.mcstas file.

\begin{verbatim}
Example 2:   ViewModISISver1(Face="Let_timeTest_155.mcstas", E0 = E_min, E1 = E_max,
\end{verbatim}

modPosition=1, xw=0.196, yh = 0.12, focus\_xw = 0.04, focus\_yh = 0.094, dist = 0.5)

LET simulation. modPosition=1 so initial surface is at front face of shutter insert.

(IMPORTANT) If modPosition=1, the xw and yh values are obsolete. The dimensions of initial surface are automatically calculated using "RDUM" values in Let\_timeTest\_155.mcstas file.
...
\midrule
\endhead
Face & string & Etable filename & "TS1\_S04\_Merlin.mcstas" \\
...
yheight &  &  & 0.12 \\
focus\_xw & m & Width of focusing rectangle & 0.094 \\
focus\_yh & m & Height of focusing rectangle & 0.094 \\
...
\subsection*{Links}
\begin{itemize}
  \item Component source code found in file \texttt{ViewModISIS.comp}.
\end{itemize}
\IfFileExists{contrib/ViewModISIS_static.tex}{\input{contrib/ViewModISIS_static.tex}}{}